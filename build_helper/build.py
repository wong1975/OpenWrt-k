...
def get_cache_restore_key(openwrt: OpenWrt, cfg: dict) -> str:
    ...
    cache_restore_key = f"{job_prefix}-{cfg["compile"]["openwrt_tag/branch"]}-{cfg["device_name"]}"
...
def prepare(cfg: dict) -> None:
    ...
    path = dl_artifact(f"openwrt-source-{cfg["device_name"]}", tmpdir.name)
    ...
    if context.job == "base-builds":
        ...
        ib_path = dl_artifact(f"Image_Builder-{cfg["device_name"]}", tmpdir.name)
...
def base_builds(cfg: dict) -> None:
    ...
    uploader.add(f"base-builds-{cfg["device_name"]}", tar_path, retention_days=1, compression_level=0)
...
def build_image_builder(cfg: dict) -> None:
    ...
    #bl_path = os.path.join(openwrt.path, "bin", "targets", target, subtarget, f"DIY-openWRT-imagebuilder-{target}-{subtarget}.Linux-x86_64.tar.zst")
    ext = "zst"
    if not os.path.exists(bl_path):
        ...
        #bl_path = os.path.join(openwrt.path, "bin", "targets", target, subtarget, f"DIY-openWRT-imagebuilder-{target}-{subtarget}.Linux-x86_64.tar.xz")
        ext = "xz"
...
def build_images(cfg: dict) -> None:
    ...
    # 压缩整个 bin 文件夹
    zip_path = os.path.join(ib.path, "bin.zip")
    shutil.make_archive(zip_path.replace('.zip', ''), 'zip', bin_path)
    logger.info(f"bin 文件夹已压缩: {zip_path}")

    # 上传压缩包
    logger.info("准备上传压缩包...")
    uploader.add(f"firmware-{cfg['name']}", zip_path, retention_days=1, compression_level=0)
    logger.info("压缩包上传完成")
